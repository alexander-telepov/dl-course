...
class LJSpeech(torch.utils.data.Dataset):
    ...
    'phonemes_start'
        `list` of `int`
        For each phoneme: the index of its first spectrogram frame.
    'phonemes_duration'
        `list` of `int`
        ...
        in between.
    'spectrogram'
        `numpy.ndarray`, shape == (`MEL_SCALE_BINS`, `sum(phonemes_durations)`)
...
def get_dataset(dataset_root):
    ...
    with open(dataset_root / 'all_possible_phoneme_codes.pkl', 'rb') as f:
        POSSIBLE_PHONEME_CODES = sorted(pickle.load(f))
...
class Vocoder:
    ...
    def __call__(self, spectrogram):
        ...
        spectrogram:
            torch.Tensor or numpy.ndarray, shape == (`MEL_SCALE_BINS`, T)
            Mel spectrogram.
...
try:
    ...
    PACKAGES_TO_INSTALL = ["git+https://github.com/shrubb/listener.git",]
    command = [sys.executable, "-m", "pip", "install"] + PACKAGES_TO_INSTALL

    print(f"`import listener` failed, trying to install by '{' '.join(command)}'...")
    subprocess.check_call(command)
    print("Successfully installed 'listener'")
